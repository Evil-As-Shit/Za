...
func _ready():
	...
	$Area2D.connect("mouse_entered", self.on_mouse_entered.bind(true))
	$Area2D.connect("mouse_exited", self.on_mouse_entered.bind(false))
...
func on_mouse_entered(b:bool):
	SignalController.emit_signal("hover_over", $Area2D, b)
